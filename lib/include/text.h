...
/* Выбирает шрифт 
 * Selects a font */
void text_set_font(BYTE font_code);
 ...
 */
BYTE text_print(BYTE page, BYTE x, const BYTE* pgmStr, BYTE xorMask);
 ...
 * xorMask - value xored to each output byte. Allows to invert ouptut.
 */
BYTE text_unum(BYTE page, BYTE x, WORD num, BYTE decpoint, BYTE xorMask);
 ...
 * xorMask - value xored to each output byte. Allows to invert ouptut.
 */
BYTE text_num(BYTE page, BYTE x, WORD num, BYTE decpoint, BYTE xorMask);
