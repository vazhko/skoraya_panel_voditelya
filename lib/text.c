...
BYTE pgm_read_byte(const BYTE *p){
    return (BYTE) *p;
}

WORD pgm_read_word(const BYTE *p){
    WORD *pw;
    pw = (WORD *)p;
    return *pw;
}

const BYTE *text_current_font = 0;

void text_set_font(BYTE font_code){
    switch(font_code){
#ifdef FONT_NORMAL
        case FONT_NORMAL: text_current_font = & font_normal;
            break;
#endif
#ifdef FONT_BOLD
        case FONT_BOLD: text_current_font = & font_bold;
            break;
#endif
#ifdef FONT_NARROW
        case FONT_NARROW: text_current_font = & font_narrow;
            break;
#endif
#ifdef FONT_HANDWRITE
        case FONT_HANDWRITE: text_current_font = & font_handwrite;
            break;
#endif
    }
}
...
BYTE _map_UTF8_to_font(unsigned char first, unsigned char second){
    if(first == 0xD0){
...
BYTE _text_output_char(BYTE x, BYTE idx, const char* pgmFont, BYTE fontFlags, BYTE fontStart, BYTE fontMax, BYTE xorMask){
    const BYTE *p;
    WORD fw;
    BYTE w;

    if(fontFlags & FONT_FLAG_BLANK_BEFORE){
        pcd8544_data(xorMask);
        x ++;
        if(x >= DISPLAY_WIDTH)
            return x;
    }
    if((idx >= fontStart) && (idx <= fontMax)){
        fw = pgm_read_word(pgmFont + 4 + (idx - fontStart) * 2);
        p = pgmFont + (fw & 0xFFF);
        w = (fw >> 12);
        ...
        if(fontFlags & FONT_FLAG_MSBUP){
            while(w --){
                pcd8544_data(reversebits(pgm_read_byte(p ++) ^ xorMask));
            }
        } else {
#endif
            while(w --){
                pcd8544_data(pgm_read_byte(p ++) ^ xorMask);
            }
#ifndef ALL_FONTS_LSB_UP
        }
#endif
...
BYTE text_print(BYTE page, BYTE x, const char* pgmStr, BYTE xorMask){
    BYTE fontFlags;
    BYTE fontStart;
    BYTE fontMax;
    char ch;
    ...
    if((x >= DISPLAY_WIDTH) || (page >= DISPLAY_PAGES) || ! text_current_font)
        return x;
    pcd8544_setpos(page, x);
    fontFlags = pgm_read_byte(text_current_font);

    fontStart = pgm_read_byte(text_current_font + 2);
    fontMax = pgm_read_byte(text_current_font + 3);
    do{
        ch = pgm_read_byte(pgmStr ++);
        if(! ch) break;
...
BYTE text_char(BYTE page, BYTE x, char ch, BYTE xorMask){
    BYTE fontFlags;
    BYTE fontStart;
    BYTE fontMax;
...
/*
const WORD _text_num_scalers[] = {1, 10, 100, 1000, 10000};

BYTE text_unum(BYTE page, BYTE x, WORD num, BYTE decpoint, BYTE xorMask){
    if((x >= DISPLAY_WIDTH) || (page >= DISPLAY_PAGES) || ! text_current_font)
        return x;
    display_setpos(page, x);
    BYTE fontFlags = pgm_read_byte(text_current_font);

    BYTE fontStart = pgm_read_byte(text_current_font + 2);
    BYTE fontMax = pgm_read_byte(text_current_font + 3);
    BYTE outme = 0;
    for(BYTE d = 4; d >= 0; d --){
        BYTE r = 0;
        WORD scaler = pgm_read_word(&_text_num_scalers[d]);
        while(num >= scaler){
            r ++;
            num -= scaler;
        }
